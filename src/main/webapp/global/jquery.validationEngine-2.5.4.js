	 ...
	 var methods = {
		...
		*/
		_validateFields: function(form, skipAjaxValidation) {
			...
			var first_err=null;
			/* Vittorio 
			* aggiungo alla condizione anche che non
			* devono essere validati al submit del form
			* i campi con id che termina per _nosave.      			
			*/
			form.find('['+options.validateAttribute+'*=validate]').not(":disabled").not('[id$="_nosave"]').each( function() {
				var field = $(this);
				var names = [];
		...
		_validateField: function(field, options, skipAjaxValidation) {
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "tableNotEmpty":
					   required = true;
					   errorMsg = methods._tableNotEmpty(field, rules, i , options);
					   break;
					case "length":
				 ...
				 $.ajax({
					 ...
					 success: function(json) {
						 ...
						 if (errorField.length == 1) {
							 ...
							 var msg = json[2];
							 if (!status) {
								 // Houston we got a incident - display an red prompt
