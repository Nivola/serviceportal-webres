 ...
 * =========================LICENSE_END==================================
 */
nivolaApp.controller('ListDnsController',
    ["$scope", "$state", "$stateParams", "$filter", "$mdDialog",
        ...
        function ($scope, $state, $stateParams, $filter, $mdDialog,
            ...
            var logger = loggers.get("ListDnsController");
            var Dns = entitiesRest.getEntity('Dns');
           ...
           $scope.limitOptions = [ 10, 20, 30];
            ...
            $scope.actions = {
                auth: {
                    new: $state.get("app.dns.new").requiredUC,
                    change: $state.get("app.dns.new").requiredUC,
                    delete: $state.get("app.dns.new").requiredUC
                },

                refresh: getDns,

                new: function () {
                    $state.go('app.formAssistenza', { object: "[" + $scope.accountName + "]" + " Richiesta di creazione DNS" });
                },
                ...
                delete: function (event) {
                    $state.go('app.formAssistenza', { object: "[" + $scope.accountName + "]" + " Richiesta di rimozione DNS" });
                }
            };
            ...
            function getDns() {
                ...
                $scope.promise = Dns.query(queryString).$promise;
                ...
                return $scope.promise.then(function (data) {
                    ...
                    if (onfail.body) {
                        if (onfail.body && onfail.body.data && onfail.body.data.message) {
                            notificationManager.showErrorPopup('Si è verificato un errore durante il caricamento dei DNS: ' + onfail.body.data.message);
                        } else {
                            notificationManager.showErrorPopup('Si è verificato un errore durante il caricamento dei DNS!');
                        }
                    } else {
                        notificationManager.showErrorPopup('Si è verificato un errore durante il caricamento dei DNS!');
                    }
