 ...
 * =========================LICENSE_END==================================
 */
nivolaApp.controller('ListCostiConsumiOrganizzazioneController',
    ["$scope", "$state", "$stateParams", "$filter", "$mdDialog", "$http","$rootScope",
        ...
        function ($scope, $state, $stateParams, $filter, $mdDialog, $http,$rootScope,
            ...
            var logger = loggers.get("ListCostiConsumiOrganizzazioneController");
            var AndamentoCosti = entitiesRest.getEntity('AndamentoCostiOrganizzazione');
	        var CostiRendicontati = entitiesRest.getEntity('CostiOrganizzazioneRendicontati');
            ...
            var RendicontoCsv = entitiesRest.getEntity('RendicontoCsv');CostiOrganizzazioneDettaglio
            var CostiOrganizzazioneDettaglio = entitiesRest.getEntity('CostiOrganizzazioneDettaglio');
           ...
           $scope.limitOptions = [ 10, 20, 30];
            ...
            $scope.init = function (idStruttura) {
                ...
                $scope.uuidStruttura =  idStruttura ? idStruttura: abilitazione.orgUuid;
                getCostiConsumi();
            };
            ...
            $scope.downloadFileCSV = function (event, rendiconto) {
                ...
                if ($scope.uuidStruttura) {
                    ...
                    queryString.tipoStruttura = 'ORGANIZZAZIONE';
                    if (AuthenticationService.getUtente().abilitazioneSelezionata.accountName){
                        queryString.nome = AuthenticationService.getUtente().abilitazioneSelezionata.orgName;
                    } else {
            ...
            function getCostiConsumi() {
                ...
                $scope.promise.then(function (data) { 
                    ...
                    var queryString = {};
                    if ($scope.uuidStruttura) {
                        queryString.idOrganizzazione = $scope.uuidStruttura;
                    }
                ...
                $scope.promise = CostiOrganizzazioneDettaglio.query(queryString).$promise;
                $scope.promise.then(function (data) {
            ...
            function impostazioniGraficoAccounts(dati) {
                ...
                $scope.graficoDivisioni = new Array(contaServizi.length).fill(0).map(() => new Array(dati.length).fill(0));
                for (var i = 0; i < dati.length; i++) {
                    let barra = new Array(dati.length).fill(0);
                    var struttura = dati[i];

                    struttura.servizi.forEach(s => {
                        $scope.graficoDivisioni[contaServizi.indexOf(s.nomeServizio)][i] = s.importo
                    });
                    $scope.options.isShowGrafico = true;
                }
