...
(function (window, angular, undefined) {
  ...
  angular.module('md-table-pagination.html', []).run(['$templateCache', function($templateCache) {
    $templateCache.put('md-table-pagination.html',
      ...
      '  <div >{{ \'paginazione.pagina\' | translate}}</div>\n' +
      '\n' +
      ...
      '  <div >{{ \'paginazione.righe\' | translate}}</div>\n' +
      '\n' +
      ...
      '  <div >{{$pagination.min()}} - {{$pagination.max()}} {{ \'paginazione.elementi_di\' | translate}} {{$pagination.total}}</div>\n' +
      '\n' +
  ...
  function mdColumn($compile, $mdUtil) {
    ...
    function postLink(scope, element, attrs, ctrls) {
      ...
      function isVisible() {
        return attrs.mdVisible === '' || scope.visible;
      }
      function setOrder() {
      ...
      scope.$watch(isVisible, function (visible) {
        updateColumn(getIndex(), {'visible': visible});
      });
  ...
  function resize($document, $window) {
    return {
      link: function(scope, elem, attrs) {
        //devo considerare la dimensione della navbar
        var sideNavWidth =  angular.element(document.querySelector("md-sidenav"))[0].clientWidth;
        var TotalFixWidth = window.innerWidth;
        var Fieldwidth = 100;	
      //  angular.element($window).bind('load', function() {
      //   delayedEvent(function() {
      //     onResize();
      //   }, 300, 'resize-table-load');  
      // });
      angular.element($window).bind('load',   onResize());
       function onResize() {
        var tablewidth = (elem[0].offsetWidth); 
        //devo considerare la dimensione della navbar
        var sideNavWidth =  angular.element(document.querySelector("md-sidenav"))[0].clientWidth;
        var totalwidth = window.innerWidth;
        if(window.innerWidth>=1300){
          totalwidth = window.innerWidth-sideNavWidth;
        }
        // console.log("sideNavWidth :"+sideNavWidth);
        // var uno = (tablewidth >= totalwidth) ? "SI":"NO";
        // console.log("tablewidth >= totalwidth "+ uno);
        // console.log(tablewidth+" >= "+totalwidth);
          ...
          if(tablewidth >= totalwidth){
            ...
            // var due = (TotalFixWidth <= totalwidth) ? "SI":"NO" ;
            // console.log("TotalFixWidth <= totalwidth "+due);
            // console.log(TotalFixWidth+" <= "+tablewidth);
            // var tre = (Fieldwidth <= totalwidth) ? "SI":"NO" ;
            // console.log("Fieldwidth <= tablewidth "+tre);
            // console.log(Fieldwidth+" <= "+tablewidth);
      ...
      // angular.element($window).on('resize',delayedEvent(function() {
      //   onResize();
      // }, 300, 'resize-data-table') );
